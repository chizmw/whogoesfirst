...
public class ScreenShot {  
    ...
    // get the screen shot of Activity锛宻ave it as png  
    ...
    public static void shoot(Activity a, String picName) {
		...
		File saveNameDir = new File(Environment.getExternalStorageDirectory().toString() + fs + "chizography" + fs + "whosfirst");
		saveNameDir.mkdirs();
		File saveNameStr = new File(saveNameDir + fs + picName + ".png");
		
        Bitmap btp = ScreenShot.takeScreenShot(a);
        if(btp != null) {
            ScreenShot.savePic(btp, saveNameStr.toString()); 
        }
        else {
